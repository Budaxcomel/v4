...
surat=$(curl -sS https://raw.githubusercontent.com/Budaxcomel/access/main/bot | grep -w 139.162.32.40 | awk '{print $4}'  )
if [[ "$surat" = "true" ]]; then
echo -ne
else
...
dircreate
        ...
        echo -e "IMMANVPN Bot Panel Installer
        "
        [[ ! -f /root/ResBotAuth ]] && {
        ...
        echo -ne "Username admin panel use '@' [Ex: @ownerimmanvpn] : "
        read admin_pnl
        [[ -z $admin_pnl ]] && admin_pnl="@ownerimmanvpn"
        echo ""
        echo -ne "Limit trial for reseller create user trial [default:1] : "
        read limit_pnl
        [[ -z $limit_pnl ]] && limit_pnl="1"
        echo ""
        echo -ne "Your name store [dafult: IMMANVPN-STORE] : "
        read store_pnl
        [[ -z $store_pnl ]] && store_pnl="IMMANVPN-STORE"
        echo ""
